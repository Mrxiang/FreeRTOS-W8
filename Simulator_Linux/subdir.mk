...
C_SRCS += $(PROJ_ROOT)/src/middle/uart8_middle.c
#util
...
#db
#C_SRCS += $(PROJ_ROOT)/src/db/DBManager.cpp
#config
C_SRCS += $(PROJ_ROOT)/src/config/config.c
...
OBJS += $(BUILD_TMP)/uart8_middle.o
OBJS += $(BUILD_TMP)/cJSON.o
OBJS += $(BUILD_TMP)/cJSON_Utils.o
#OBJS += $(BUILD_TMP)/DBManager.o
OBJS += $(BUILD_TMP)/config.o
#
...
C_DEPS += $(BUILD_TMP)/uart8_middle.d
C_DEPS += $(BUILD_TMP)/cJSON.d
C_DEPS += $(BUILD_TMP)/cJSON_Utils.d
#C_DEPS += $(BUILD_TMP)/DBManager.d
C_DEPS += $(BUILD_TMP)/config.d
#OBJS= $(C_SRCS:.cpp=.o)
#C_DEPS= $(OBJS:.o=.d)
...
$(BUILD_TMP)/%.o: $(PROJ_ROOT)/src/config/%.c
	@echo 'Building file: $<'
	gcc -DUSE_STDIO=1 -D__GCC_POSIX__=1 $(TARGET_INC) -O2 -Wall -c -fmessage-length=0 -pthread -lrt -MMD -MP -MF"$(@:%.o=%.d)" -MT"$(@:%.o=%.d)" -o"$@" "$<"
	@echo 'Finished building: $<'
	@echo ' '
#$(BUILD_TMP)/%.o: $(PROJ_ROOT)/src/db/%.cpp
#	@echo 'Building file: $<'
#	gcc -DUSE_STDIO=1 -D__GCC_POSIX__=1 $(TARGET_INC) -O2 -Wall -c -fmessage-length=0 -pthread -lrt -MMD -MP -MF"$(@:%.o=%.d)" -MT"$(@:%.o=%.d)" -o"$@" "$<"
#	@echo 'Finished building: $<'
#	@#echo ' '
